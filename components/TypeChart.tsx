...
export default function TypeChart( props: Props ) {
        ...
        <table className='mx-auto text-center text-gray-200 overflow-x-scroll'>
          <thead>
            <tr>
              ...
              { types.map(type => <th key={type} className={`${cell} bg-gray-900 text-sm`} > {capitalize(type)} </th>) }
            </tr>
          </thead>
          <tbody>
            { types.map((t, idx1) => 
              <tr key={idx1}>
                <td className={`${cell} text-sm bg-gray-900`} > {capitalize(t)} </td>
                {types.map((type, idx2) => {
